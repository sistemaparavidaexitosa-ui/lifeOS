...
| Proyectos y Tareas | `/execution` | `execution/page.tsx`; dependencias entre proyectos en `ProjectDepsField.tsx` (dentro del menú del proyecto → «Editar») | `projects` (+`depends_on` desde 0056),`tasks` | `createProject`,`createTask`,`setTaskStatus`,`setProjectDeps` |
| Mapa de dependencias (Execution Graph) · 7 vistas | `/graph?view=project\|workspace\|knowledge\|personal\|money\|ai\|impact` (+`?node=` o `?entity=` para fijar la raíz) | `graph/page.tsx` (servidor) → `components/graph/GraphWorkspace.tsx` → `GraphCanvas.tsx` (único que toca `<canvas>`), `GraphToolbar`, `GraphSearch`, `Minimap`, `NodeInspector`; motor puro en `lib/domain/graph/**` | `graph_nodes`, `graph_edges`, `graph_layouts`, `graph_node_types`, `graph_rel_types` (0054) — proyectadas por trigger desde 14 tablas de dominio. Lectura vía RPC `graph_subgraph`/`graph_impact`/`graph_edges_of`/`graph_search` (nunca `.from()`: `max_rows = 1000` recortaría en silencio) | `createGraphEdge`,`deleteGraphEdge`,`createCustomNode`,`deleteCustomNode`,`saveLayout`,`loadLayout`,`searchGraph`,`fetchImpact`; `GET /api/graph/subgraph` para la expansión progresiva desde el navegador |
...
| Panel de Desarrollo Personal | `/development` | `development/page.tsx` | `personal_goals`,`key_results`,`routines`,`habits` | Lectura agregada |
...
| Rutinas y Hábitos · Hoy | `/development/routines` | `development/routines/{layout,RoutineTabs,page}.tsx`, `identity/{IdentityHero,IdentityScoreCard,IdentityProfileSheet,TraitManager}.tsx`, `brief/{BriefCard,BriefGenerator,VisualizationPlayer}.tsx`, `RoutineForm.tsx`,`HabitForm.tsx`,`HabitRow.tsx`,`HabitLogSheet.tsx`,`DailyCheckinCard.tsx`,`RoutineTemplates.tsx`,`HabitTemplates.tsx`, `InsightSection` | `routines` (+`identity`),`habits`,`habit_logs` (+`status`,`completion_pct`,`note`,`mood`,`energy`, 0063),`routine_runs`,`occupations`,`daily_reflections` (0063),`identity_profiles`,`identity_traits`,`habit_identity_traits`,`identity_scores` (0064),`identity_briefs` (0065),`recommendations`; series por `habit_log_series` | `toggleHabitToday`,`logHabit`,`saveDailyReflection`,`upsertRoutine`,`deleteRoutine`,`upsertHabit` (+votos),`deleteHabit`,`createRoutineFromTemplate`,`upsertIdentityProfile`,`upsertTrait`,`deleteTrait`,`generateTodayBrief`,`regenerateTodayBrief`,`reactToBriefItem`,`analyze("habits")` |
| Rutinas y Hábitos · Analítica | `/development/routines/analytics` | `analytics/{page,RangePicker,HabitTable}.tsx`, `components/charts/*` (Recharts, D-158) | Las mismas series más `identity_scores`; cálculo en `lib/domain/development/habit-dashboard.ts` y `lib/domain/identity/score.ts` | Solo lectura; `?rango=7\|30\|90\|365` |
| Nutrición | `/development/nutrition` | `development/nutrition/page.tsx`,`FoodSearchForm.tsx`,`BodyProfileForm.tsx`,`WeightForm.tsx`,`EntryRow.tsx` | `nutrition_profiles`,`food_entries`,`body_measurements`,`foods`,`journal_entries` (solo el conteo del mes, sin join) | `upsertBodyProfile`,`upsertWeight`,`logFoodEntry`,`updateFoodEntry`,`deleteFoodEntry`; `GET /api/development/food-lookup` |
| (redirect) | `/development/habits` | `development/habits/page.tsx` | — | redirige a `/development/routines` desde 0046 |
...
| Chat de IA transversal (rail lateral, todas las pantallas) | Sin ruta: tercera columna del `AppShell` desde 1280px; burbuja + hoja por debajo | `components/AiChatRail.tsx`, `lib/ai-chat/actions.ts`, `lib/ai/chat.ts`, `lib/domain/ai/chat.ts` | `ai_chat_messages` (0045); contexto vía `lib/insights/{facts-loader,context}.ts` filtrado por `profiles.ai_domains` | `loadChatHistory`,`sendChatMessage`,`createTaskFromChat`,`clearChat` |
