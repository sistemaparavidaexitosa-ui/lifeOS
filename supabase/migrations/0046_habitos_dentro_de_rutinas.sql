-- 0046_habitos_dentro_de_rutinas.sql
--
...
comment on table public.habit_logs is
  'FR-HAB-002: un registro por (habit_id, log_date). Calcula la racha y, desde 0046, es también la única fuente de «¿hice hoy este paso?». Dejó de ser append-only en esa misma migración (D-094): cuando el paso y el hábito eran dos registros, desmarcar el paso no podía borrar la racha; ahora son el mismo registro, así que desmarcar borra su fila.';

comment on table public.routine_runs is
  'Único por (routine_id, local_date). Ya no lleva la lista de pasos hechos: eso vive en habit_logs desde 0046. Sobrevive por started_at y completed_at, que dicen cuándo arrancaste la rutina y cuándo la cerraste — dato que ninguna otra tabla tiene.';
