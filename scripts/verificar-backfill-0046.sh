#!/usr/bin/env bash
# Comprueba que el backfill de 0046 coloca los datos legados donde toca.
#
...
# aplica 0046 a mano y comprueba el resultado.
#
# CUIDADO: hace `supabase db reset`. Borra la base LOCAL.
set -euo pipefail

MIG="supabase/migrations/0046_habitos_dentro_de_rutinas.sql"
...
restaurar() {
  [ -f "$TMP/0046.sql" ] && mv "$TMP/0046.sql" "$MIG"
...
echo "→ Reconstruyendo la base en el estado anterior a 0046…"
supabase db reset

echo "→ Sembrando datos con la forma vieja…"
correr scripts/backfill/0046_fixture.sql

echo "→ Aplicando 0046 (en una sola transacción)…"
...
correr_atomico "$TMP/0046.sql"

echo "→ Comprobando…"
correr scripts/backfill/0046_asserts.sql
