...
export async function getHomeData(userId: string) {
  ...
  const [
    ...
    supabase.from("books").select("*").eq("user_id", userId).eq("status", "Leyendo").order("updated_at", { ascending: false }).limit(1).maybeSingle()
  ]);
  ...
  const saturation = saturationStatus(
    ...
    (occupations ?? []).map((o) => ({ id: o.id, title: o.title, start: o.start_time.slice(0, 5), end: o.end_time.slice(0, 5) })),
    impactMinutes
  );
