...
import { sanitizeProposedTask, type ChatMessageLike } from "@/lib/domain/ai/chat.ts";
...
const REPLY_RESPONSE_SCHEMA: GeminiSchema = {
  ...
  required: ["text", "factIds", "proposedTaskTitle"],
  propertyOrdering: ["text", "factIds", "proposedTaskTitle"]
};
...
export async function chatReply(input: ChatInput): Promise<ChatResult> {
  ...
  if (!message) return { ok: false, text: "", factIds: [], proposedTask: null, reason: "Escribe algo primero." };
  ...
  if (!result.ok || !result.data) {
    return { ok: false, text: "", factIds: [], proposedTask: null, reason: result.reason };
  }

  const text = result.data.text.trim();
  if (!text) {
    return { ok: false, text: "", factIds: [], proposedTask: null, reason: "El modelo devolvió una respuesta vacía." };
  }
  ...
  const conocidos = new Set(input.context.facts.map((f) => f.id));
  const factIds = [...new Set(result.data.factIds.filter((id) => conocidos.has(id)))];

  return { ok: true, text, factIds, proposedTask: sanitizeProposedTask(result.data.proposedTaskTitle) };
}
