...
import ProjectDepsField from "./ProjectDepsField";
...
export default function EditProjectForm({ project, onSaved }: { project: ProjectMenuData; onSaved: () => void }) {
  ...
  return (
    <>
      ...
      {/* FUERA del <form> a propósito. Las dependencias se guardan solas al
          marcar una casilla, con su propia acción y su propio mensaje: el fallo
          esperable ahí es un ciclo, y un ciclo hay que explicarlo. Dentro del
          formulario, ese mensaje se perdería entre el resto de campos y no se
          sabría cuál de ellos falló. */}
      <ProjectDepsField
        projectId={project.id}
        candidates={project.siblings}
        selected={project.dependsOn}
      />
    </>
  );
}
